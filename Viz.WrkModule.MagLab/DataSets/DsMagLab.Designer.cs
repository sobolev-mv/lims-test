...
namespace Viz.Lims.MagLab.DataSets {
    ...
    public partial class DsMagLab : Devart.Data.Oracle.OracleDataSet {
        ...
        public partial class MlSamplesDataTable : Devart.Data.Oracle.OracleDataTable, System.Collections.IEnumerable {
            ...
            [System.Diagnostics.DebuggerNonUserCodeAttribute()]
            public MlSamplesRow AddMlSamplesRow(string SampleId, string TestType, string SteelType, decimal ThickNessNominal, string Line, int LaserFlag, string SamplePos, string MatLocalNumber, string MatMarkingInfo, System.DateTime DtSample, decimal State, string ErrText) {
            ...
            private void InitClass() {
                ...
                this.columnDtSample.Caption = "DTSAMPLE";
                ...
                this.SelectCommand = new Devart.Data.Oracle.OracleCommand(@"SELECT  SAMPLEID, 
CASE 
  WHEN TESTTYPE = 'PL' THEN 'ОП' 
  WHEN TESTTYPE = 'RE' THEN 'ПП' 
  WHEN TESTTYPE = 'CO' THEN 'КП' 
  WHEN TESTTYPE = 'PE' THEN 'ПРП'  
  ELSE '!?' 
END TESTTYPE, 
CASE 
  WHEN STEELTYPE = 'GO' THEN 'АН' 
  ELSE 'ИЗ' 
END STEELTYPE, THICKNESSNOMINAL, LINE, 
CASE 
  WHEN LASERFLAG = 'Y' THEN 1
  ELSE 0
END LASERFLAG, 
CASE 
  WHEN SAMPLEPOS = 'L' THEN 'Н' 
  WHEN SAMPLEPOS = 'T' THEN 'К' 
  WHEN SAMPLEPOS = 'M' THEN 'С' 
  ELSE '!?' 
END SAMPLEPOS, MATLOCALNUMBER, MATMARKINGINFO, DTSAMPLE, STATE, ERR_TEXT 
FROM  LIMS.SAMPLEMEAS 
WHERE (DTSAMPLE BETWEEN :DT1 AND :DT2) 
      AND ((SAMPLEID LIKE :SMPID || '%') OR (:SMPID = 'Z')) 
      AND ((MATMARKINGINFO LIKE :MARKINFO || '%') OR (:MARKINFO = 'Z')) 
      AND ((MATLOCALNUMBER LIKE :MATLOC || '%') OR (:MATLOC = 'Z')) 
ORDER BY  MATLOCALNUMBER, SAMPLEPOS
", Connection);
                ...
                this.TableMapping.ColumnMappings.Add("THICKNESS_NOMINAL", "ThickNessNominal");
                this.TableMapping.ColumnMappings.Add("LINE", "Line");
                this.TableMapping.ColumnMappings.Add("LASER_FLAG", "LaserFlag");
                this.TableMapping.ColumnMappings.Add("SAMPLE_POS", "SamplePos");
