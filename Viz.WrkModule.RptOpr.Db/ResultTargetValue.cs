...
{
  public sealed class ResultTargetValueRptParam : Smv.Xls.XlsInstanceParam
  {
    public DateTime DateBegin { get; set; }
    public DateTime DateEnd { get; set; }
    public ResultTargetValueRptParam(string sourceXlsFile, string destXlsFile) : base(sourceXlsFile, destXlsFile)
    {}
  }

  public sealed class ResultTargetValue : Smv.Xls.XlsRpt
  {

    protected override void DoWorkXls(object sender, DoWorkEventArgs e)
    {
      var prm = (e.Argument as ResultTargetValueRptParam);
    ...
    private Boolean RunRpt(ResultTargetValueRptParam prm, dynamic CurrentWrkSheet)
    {
      ...
      try{
        ...
        //CurrentWrkSheet.Range["H1:L1"].ClearContents();
        CurrentWrkSheet.Cells[2, 1].Value = $"Выгрузка по дате фактического назначения за период:  с {dtBegin:dd.MM.yyyy HH:mm:ss} по {dtEnd:dd.MM.yyyy HH:mm:ss}";

        const string sqlStmt1 = "SELECT * FROM VIZ_PRN.V_RESULT_TV";
